...
if [ $ref == "hg38" ]; then
  ...
  $samtools_bin view -b $bam_path chr6:29720000-33130000 \
...
if [ $ref == "hg19" ];then
	...
	$samtools_bin view -b $bam_path chr6:29690000-33103000 \
	chr6_apd_hap1 chr6_cox_hap2 chr6_dbb_hap3 chr6_mann_hap4 chr6_mcf_hap5 chr6_qbl_hap6 chr6_ssto_hap7 | $samtools_bin view -b -F 0x4 - | $samtools_bin sort --thread $num_processors -m $samtools_sort_memory_per_thread -O BAM - > $sampleid.tmp.extract.bam
...
if [ ! $OUT -eq 0 ];then
    echo 'Something went wrong while extracting HLA-related reads '
    exit 1
fi


echo ">>>>>>>>>>>>>> indexing extracted bam "
$samtools_bin index $sampleid.tmp.extract.bam


echo ">>>>>>>>>>>>>> bamUtil fastq extraction "
$bamUtil bam2FastQ --in $sampleid.tmp.extract.bam --gzip --firstOut $sampleid\_extract_1.fq.gz --secondOut $sampleid\_extract_2.fq.gz --unpairedOut $sampleid\_extract.unpaired.fq.gz &> /dev/null

OUT=$?
if [ ! $OUT -eq 0 ];then
    echo '$bamUtil fastq extraction Failed! '
    exit 1
# else
