...
# :<<!
for hla in ${HLAs[@]}; do
    config_file=$dir/db/complex/${hla}.config.txt
    echo bwa=$dir/db/complex/${hla}/${hla}.fa >$config_file
    echo freebayes=$dir/db/complex/${hla}/${hla}.fa >>$config_file
    echo blat=$dir/db/complex/${hla}/ >>$config_file
done
...
bowtie2-build $dir/db/ref/merged.fa $dir/db/ref/merged.fa
...
if [ -f "$license" ];then
    ...
    $dir/bin/novoindex  -k 14 -s 1 $dir/db/ref/merged.ndx \
    $dir/db/ref/merged.fa
fi
